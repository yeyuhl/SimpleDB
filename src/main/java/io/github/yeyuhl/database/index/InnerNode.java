...
/**
 * B+ 树的内部节点。顺序为 d 的 B+ 树中的每个内部节点都存储在 d 和 2d 键之间。具有 n 个键的内部节点存储指向子节点的 n + 1 个“指针”（其中指针只是一个页码）。
 * 此外，每个内部节点都序列化并持久化在单个页面上;有关如何序列化内部节点的详细信息，请参阅 toBytes 和 fromBytes。例如，下面是阶 2 内节点的图示：
